...
#[test_log::test]
fn test_concurrent_write_barriers() -> Result<()> {
    let collector = GarbageCollector::new();
    collector.start();
    ...
    // Add root to keep nodes reachable
    nodes[0].as_root(&collector)?;

    let mut handles = vec![];

    // Spawn threads that create references between nodes
    for thread_id in 0..3 {
        let thread_nodes = nodes.clone();
        let collector = collector.clone();
        let handle = thread::spawn(move || {
            for i in 0..50 {
                let from_idx = (thread_id * 50 + i) % thread_nodes.len();
                let to_idx = (from_idx + 1) % thread_nodes.len();

                let from_node = &thread_nodes[from_idx];
                let to_node = &thread_nodes[to_idx];

                // Create reference and trigger write barrier
                if let Ok(mut refs) = from_node.references.lock() {
                    refs.push(to_node.clone());
                    to_node.write_barrier(&collector);
                }

public class Test {
    ...
    public static Move findBestMoveAlphaBeta(int[][] board) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (board[i][j] == EMPTY) {
                    board[i][j] = PLAYER_X;
                    int moveScore = alphaBeta(board, 0, false, Integer.MIN_VALUE, Integer.MAX_VALUE);
                    board[i][j] = EMPTY;
    ...
    private static int alphaBeta(int[][] board, int depth, boolean isMaximizing, int alpha, int beta) {
    ...
    public static void main(String[] args) {
        ...
        Move bestMove = findBestMoveAlphaBeta(board);
        System.out.println("Best move for X (Alpha-Beta): (" + bestMove.row + ", " + bestMove.col + ")");

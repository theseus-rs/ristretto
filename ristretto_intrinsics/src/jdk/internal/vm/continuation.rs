...
pub fn do_yield<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn enter_special<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn is_pinned_0<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn pin<T: Thread + 'static>(_thread: Arc<T>, _parameters: Parameters) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_do_yield() {
        ...
        let result = do_yield(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_is_pinned_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = is_pinned_0(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
            "jdk.internal.vm.Continuation.isPinned0(Ljdk/internal/vm/ContinuationScope;)I",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_pin() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = pin(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_unpin() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = unpin(thread, Parameters::default());
        assert_eq!(

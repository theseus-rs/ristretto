...
pub fn close_registry_key<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn compare_paths<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn enum_registry_value<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn open_registry_key<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn read_dword_value<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_close_registry_key() {
        ...
        let result = close_registry_key(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(
            "jdk/jpackage/internal/WindowsRegistry.closeRegistryKey(J)V",
    ...
    async fn test_enum_registry_value() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result =
            enum_registry_value(thread, Parameters::new(vec![Value::Long(0), Value::Int(0)]));
        assert_eq!(
            "jdk/jpackage/internal/WindowsRegistry.enumRegistryValue(JI)Ljava/lang/String;",
            result.unwrap_err().to_string()
        );

...
pub fn icon_swap<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn lock_resource<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn unlock_resource<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _j_resource_lock = parameters.pop_long()?;
...
pub fn unlock_resource_windows_v25<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _j_resource_lock = parameters.pop_long()?;
...
pub fn version_swap<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_lock_resource() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = lock_resource(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
            "jdk/jpackage/internal/ExecutableRebrander.lockResource(Ljava/lang/String;)J",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_unlock_resource() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = unlock_resource(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(
            "jdk/jpackage/internal/ExecutableRebrander.unlockResource(J)V",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_unlock_resource_windows_v25() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = unlock_resource_windows_v25(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(
            "jdk/jpackage/internal/ExecutableRebrander.unlockResource(J)Z",
            result.unwrap_err().to_string()
        );

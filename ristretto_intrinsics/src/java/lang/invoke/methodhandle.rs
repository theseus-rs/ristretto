...
async fn box_primitive<T: Thread + 'static>(thread: &T, value: Value) -> Result<Value> {
    match value {
...
mod tests {
    ...
    async fn create_test_member_name<T: Thread + 'static>(
        thread: &T,
        class_name: &str,
        member_name: &str,
        descriptor: &str,
    ...
    async fn register_method_handle_class<T: Thread + 'static>(thread: &T) -> Result<Arc<Class>> {
    ...
    #[tokio::test]
    async fn test_constructor_class_helpers() -> Result<()> {
        assert_eq!(
            Some("java/lang/String".to_string()),
            object_return_class_name("(I)Ljava/lang/String;")
        );
        assert_eq!(None, object_return_class_name("(I)V"));
        assert_eq!(None, object_return_class_name("invalid"));

        let (_vm, thread) = crate::test::thread().await?;
        let object_class = thread.class("java/lang/Object").await?;
        let string_class = thread.class("java/lang/String").await?;
        assert!(is_same_or_subclass_of(&string_class, &object_class)?);
        assert!(is_same_or_subclass_of(&string_class, &string_class)?);
        assert!(!is_same_or_subclass_of(&object_class, &string_class)?);

        let vm = thread.vm()?;
        let collector = vm.garbage_collector();
        let object = Value::new_object(collector, Reference::Object(Object::new(object_class)?));
        assert_eq!(None, direct_constructor_instance_class_name(&object)?);
        assert_eq!(None, method_handle_return_class_name(&object)?);

        let constructor_class = thread
            .class("java/lang/invoke/DirectMethodHandle$Constructor")
            .await?;
        let constructor = Value::new_object(
            collector,
            Reference::Object(Object::new(constructor_class)?),
        );
        assert_eq!(None, direct_constructor_instance_class_name(&constructor)?);
        ...
        assert_eq!(None, method_handle_return_class_name(&method_handle)?);
        Ok(())
    }
}

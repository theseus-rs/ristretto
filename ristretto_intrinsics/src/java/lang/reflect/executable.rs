...
pub async fn get_parameters_0<T: Thread + 'static>(
    ...
    let (class_name, slot) = {
        ...
        let slot = usize::try_from(slot_i32).map_err(|_| {
            ristretto_types::Error::InternalError(format!("Invalid slot value: {slot_i32}"))
        })?;
    ...
    let method = methods.get(slot).ok_or_else(|| {
        ristretto_types::Error::InternalError(format!("Method slot {slot} out of bounds"))
    })?;
...
pub async fn get_type_annotation_bytes_0<T: Thread + 'static>(
    ...
    // Create byte array
    let bytes: Vec<i8> = bytes.into_iter().map(u8::cast_signed).collect();

...
#[cfg(not(target_family = "wasm"))]
fn register_midi_out<T: ristretto_types::Thread + 'static>(
    thread: &Arc<T>,
    connection: super::audio_state::MidiOutConnection,
) -> Result<Option<Value>> {
    let audio = thread
        .vm()?
        .resource_manager()
...
pub async fn n_open<T: ristretto_types::Thread + 'static>(
    ...
    #[cfg(not(target_family = "wasm"))]
    {
        let midi_out = midir::MidiOutput::new("ristretto").ok();
        let index = index as usize;

        #[cfg(target_os = "macos")]
        if index == midi_out.as_ref().map_or(0, midir::MidiOutput::port_count) {
            let Ok(synth) = super::dls_synth::DlsSynth::new() else {
                return Ok(Some(Value::Long(0)));
            };
            return register_midi_out(
                &thread,
                super::audio_state::MidiOutConnection::DlsSynth(synth),
            );
        }
        ...
        register_midi_out(
            &thread,
            super::audio_state::MidiOutConnection::Device(connection),
        )
    }
    ...
    #[cfg(target_os = "wasi")]
    {
        ...
        let audio = thread
            ...
            .get_or_init(super::audio_state::AudioState::new)?;
        let handle_id = audio.next_midi_out_id();
        let midi_handle = super::audio_state::MidiOutHandle { start_time_ms: 0.0 };
        audio
            .midi_out_handles()
            .write()
            .insert(handle_id, midi_handle);
...
pub async fn n_send_long_message<T: ristretto_types::Thread + 'static>(
    ...
    #[cfg(not(target_family = "wasm"))]
    {
            ...
            && let Ok(bytes) = data.as_bytes()
        {
            ...
            if let Some(message) = bytes.get(..end) {
                match conn {
                    super::audio_state::MidiOutConnection::Device(connection) => {
                    ...
                    #[cfg(target_os = "macos")]
                    super::audio_state::MidiOutConnection::DlsSynth(synth) => {
                        let _ = synth.send_system_exclusive(message);
                    }
                }
            }
        }
    ...
    #[cfg(target_os = "wasi")]
    let _ = (thread, length, data, handle_id);
...
pub async fn n_send_short_message<T: ristretto_types::Thread + 'static>(
    ...
    #[cfg(not(target_family = "wasm"))]
    {
            ...
            && let Some(ref mut conn) = *conn_guard
        {
            ...
            match conn {
                ...
                #[cfg(target_os = "macos")]
                super::audio_state::MidiOutConnection::DlsSynth(synth) => {
                    let _ = synth.send_short_message(status, data1, data2);
                }
            }
        }
    }
    ...
    #[cfg(target_os = "wasi")]
    let _ = (thread, packed_msg, handle_id);

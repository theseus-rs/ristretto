...
#[cfg(not(target_family = "wasm"))]
pub(crate) enum MidiOutConnection {
    Device(midir::MidiOutputConnection),
    #[cfg(target_os = "macos")]
    DlsSynth(super::dls_synth::DlsSynth),
}

#[cfg(not(target_family = "wasm"))]
pub(crate) struct MidiOutHandle {
    pub connection: Mutex<Option<MidiOutConnection>>,
    pub start_time: std::time::Instant,
}
...
pub(crate) fn get_midi_in_port_count() -> usize {
    midir::MidiInput::new("ristretto-probe").map_or(0, |midi_in| midi_in.port_count())
}
...
pub(crate) fn get_midi_out_port_count() -> usize {
    let port_count =
        midir::MidiOutput::new("ristretto-probe").map_or(0, |midi_out| midi_out.port_count());
    #[cfg(target_os = "macos")]
    return port_count + 1;
...
pub(crate) fn get_midi_out_port_name(index: usize) -> Option<String> {
    ...
    #[cfg(target_os = "macos")]
    let port_count = midi_out.as_ref().map_or(0, midir::MidiOutput::port_count);
    if let Some(midi_out) = midi_out {
        let ports = midi_out.ports();
        if let Some(port) = ports.get(index) {
            return midi_out.port_name(port).ok();
        }
    }
    #[cfg(target_os = "macos")]
    if index == port_count {
        return Some("Apple DLS Synth".to_string());
    }
    None
}
...
/// WASI has no browser Performance API, so headless MIDI handles use a zero timestamp.
#[cfg(target_os = "wasi")]
pub(crate) fn performance_now_micros() -> i64 {
    0
}

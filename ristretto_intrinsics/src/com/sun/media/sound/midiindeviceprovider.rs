...
pub async fn n_get_description<T: ristretto_types::Thread + 'static>(
    ...
    {
        if let Some(name) = super::audio_state::get_midi_in_port_name(index as usize) {
    ...
    {
        let _ = index;
        Ok(Some("Web MIDI Input".to_object(&thread).await?))
    }
}
...
pub async fn n_get_num_devices<T: ristretto_types::Thread + 'static>(
    ...
    #[cfg(not(target_family = "wasm"))]
    {
        let count = super::audio_state::get_midi_in_port_count() as i32;

...
pub fn end_aot_recording<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn get_available_processors<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_class_initialization_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_class_loading_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_class_verification_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_daemon_thread_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_initialized_class_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_live_thread_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_loaded_class_size<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_method_data_size<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_peak_thread_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_process_id<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_safepoint_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_safepoint_sync_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_startup_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_total_application_non_stopped_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_total_class_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_total_compile_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_total_safepoint_time<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_total_thread_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_unloaded_class_count<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_unloaded_class_size<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_uptime_0<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_verbose_class<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_verbose_gc<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_version_0<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_vm_arguments_0<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn init_optional_support_fields<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn is_thread_allocated_memory_enabled<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn is_thread_contention_monitoring_enabled<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn is_thread_cpu_time_enabled<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
mod tests {
    ...
    async fn test_end_aot_recording() {
        ...
        let result = end_aot_recording(thread, Parameters::default());
        assert_eq!(
            "sun.management.VMManagementImpl.endAOTRecording()Z",
    ...
    async fn test_get_available_processors() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_available_processors(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_class_initialization_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_class_initialization_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_class_loading_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_class_loading_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_class_verification_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_class_verification_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_daemon_thread_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_daemon_thread_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_initialized_class_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_initialized_class_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_live_thread_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_live_thread_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_loaded_class_size() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_loaded_class_size(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_method_data_size() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_method_data_size(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_peak_thread_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_peak_thread_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_process_id() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_process_id(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_safepoint_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_safepoint_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_safepoint_sync_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_safepoint_sync_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_startup_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_startup_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_total_application_non_stopped_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_total_application_non_stopped_time(thread, Parameters::default());
        assert_eq!(
            "sun.management.VMManagementImpl.getTotalApplicationNonStoppedTime()J",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_get_total_class_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_total_class_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_total_compile_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_total_compile_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_total_safepoint_time() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_total_safepoint_time(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_total_thread_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_total_thread_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_unloaded_class_count() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_unloaded_class_count(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_unloaded_class_size() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_unloaded_class_size(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_uptime_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_uptime_0(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_verbose_class() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_verbose_class(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_verbose_gc() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_verbose_gc(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_version_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_version_0(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_vm_arguments_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_vm_arguments_0(thread, Parameters::default());
        assert_eq!(
            "sun.management.VMManagementImpl.getVmArguments0()[Ljava/lang/String;",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_init_optional_support_fields() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = init_optional_support_fields(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_is_thread_allocated_memory_enabled() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = is_thread_allocated_memory_enabled(thread, Parameters::default());
        assert_eq!(
            "sun.management.VMManagementImpl.isThreadAllocatedMemoryEnabled()Z",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_is_thread_contention_monitoring_enabled() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = is_thread_contention_monitoring_enabled(thread, Parameters::default());
        assert_eq!(
            "sun.management.VMManagementImpl.isThreadContentionMonitoringEnabled()Z",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_is_thread_cpu_time_enabled() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = is_thread_cpu_time_enabled(thread, Parameters::default());
        assert_eq!(

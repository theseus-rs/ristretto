...
pub fn add_subview<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn remove_from_superview<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _view_ptr = parameters.pop_long()?;
...
pub fn set_frame<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_hidden<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_remove_from_superview() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = remove_from_superview(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(
    ...
    async fn test_window() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = window(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(

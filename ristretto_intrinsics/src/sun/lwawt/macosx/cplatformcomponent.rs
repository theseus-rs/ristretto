...
pub fn native_create_component<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn native_set_bounds<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_native_create_component() {
        ...
        let result = native_create_component(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(

...
pub fn fallback_domain_0<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn local_domain_0<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn load_dnsconfig0<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn notify_addr_change0<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
mod tests {
    ...
    async fn test_fallback_domain_0() {
        ...
        let result = fallback_domain_0(thread, Parameters::default());
        assert_eq!(
            "sun.net.dns.ResolverConfigurationImpl.fallbackDomain0()Ljava/lang/String;",
    ...
    async fn test_local_domain_0() {
        ...
        let result = local_domain_0(thread, Parameters::default());
        assert_eq!(
            "sun.net.dns.ResolverConfigurationImpl.localDomain0()Ljava/lang/String;",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_load_dnsconfig0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = load_dnsconfig0(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_notify_addr_change0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = notify_addr_change0(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_init0_windows() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = init0_windows(thread, Parameters::default());
        assert_eq!(
            "sun/net/dns/ResolverConfigurationImpl.init0()V",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_load_dnsconfig0_windows() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = load_dnsconfig0_windows(thread, Parameters::default());
        assert_eq!(
            "sun/net/dns/ResolverConfigurationImpl.loadDNSconfig0()V",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_notify_addr_change0_windows() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = notify_addr_change0_windows(thread, Parameters::default());
        assert_eq!(
            "sun/net/dns/ResolverConfigurationImpl.notifyAddrChange0()I",
            result.unwrap_err().to_string()
        );

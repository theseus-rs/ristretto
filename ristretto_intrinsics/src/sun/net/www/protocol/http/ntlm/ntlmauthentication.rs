...
pub fn is_trusted_site0<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn is_trusted_site_available<T: Thread + 'static>(
    _thread: Arc<T>,
...
mod tests {
    ...
    async fn test_is_trusted_site0() {
        ...
        let result = is_trusted_site0(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
            "sun/net/www/protocol/http/ntlm/NTLMAuthentication.isTrustedSite0(Ljava/lang/String;)Z",
    ...
    async fn test_is_trusted_site_available() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = is_trusted_site_available(thread, Parameters::default());
        assert_eq!(
            "sun/net/www/protocol/http/ntlm/NTLMAuthentication.isTrustedSiteAvailable()Z",
            result.unwrap_err().to_string()
        );

...
pub fn get_default_pix_fmt<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn get_oglcapabilities<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn get_wglconfig_info<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn init_wgl<T: Thread + 'static>(
    _thread: Arc<T>,
...
mod tests {
    ...
    async fn test_get_default_pix_fmt() {
        ...
        let result = get_default_pix_fmt(thread, Parameters::new(vec![Value::Int(0)]));
        assert_eq!(
    ...
    async fn test_get_oglcapabilities() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_oglcapabilities(thread, Parameters::new(vec![Value::Long(0)]));
        assert_eq!(
    ...
    async fn test_get_wglconfig_info() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result =
            get_wglconfig_info(thread, Parameters::new(vec![Value::Int(0), Value::Int(0)]));
        assert_eq!(
    ...
    async fn test_init_wgl() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = init_wgl(thread, Parameters::default());
        assert_eq!(

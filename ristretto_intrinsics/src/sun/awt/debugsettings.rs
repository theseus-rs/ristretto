...
pub fn set_c_tracing_on_1<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn set_c_tracing_on_2<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_c_tracing_on_3<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_set_c_tracing_on_1() {
        ...
        let result = set_c_tracing_on_1(thread, Parameters::new(vec![Value::from(false)]));
        assert_eq!(

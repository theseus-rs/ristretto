...
pub fn check_gtk<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn get_gtk_version<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn gtk_check_version_impl<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn load_gtk<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn load_gtk_icon<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn load_stock_icon<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn unload_gtk<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn check_gtk_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _version = parameters.pop_int()?;
...
pub fn get_gtk_version_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(
...
pub fn gtk_check_version_impl_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _micro = parameters.pop_int()?;
...
pub fn load_gtk_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _verbose = parameters.pop_bool()?;
...
pub fn load_gtk_icon_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _filename = parameters.pop_reference()?;
...
pub fn load_stock_icon_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _detail = parameters.pop_reference()?;
...
pub fn native_sync_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError("sun/awt/UNIXToolkit.nativeSync()V".to_string()).into())
...
pub fn unload_gtk_linux_ge_v11<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError("sun/awt/UNIXToolkit.unload_gtk()Z".to_string()).into())
...
mod tests {
    ...
    async fn test_check_gtk() {
        ...
        let result = check_gtk(thread, Parameters::new(vec![Value::Int(0)]));
        assert_eq!(
    ...
    async fn test_get_gtk_version() {
        let (_vm, thread) = crate::test::java8_thread().await.expect("thread");
        let result = get_gtk_version(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_load_gtk_icon() {
        let (_vm, thread) = crate::test::java8_thread().await.expect("thread");
        let result = load_gtk_icon(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
            "sun.awt.UNIXToolkit.load_gtk_icon(Ljava/lang/String;)Z",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_unload_gtk() {
        let (_vm, thread) = crate::test::java8_thread().await.expect("thread");
        let result = unload_gtk(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_check_gtk_linux_ge_v11() {
        ...
        let result = check_gtk_linux_ge_v11(thread, Parameters::new(vec![Value::Int(0)]));
        assert_eq!(
            "sun/awt/UNIXToolkit.check_gtk(I)Z",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_get_gtk_version_linux_ge_v11() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_gtk_version_linux_ge_v11(thread, Parameters::default());
        assert_eq!(
            "sun/awt/UNIXToolkit.get_gtk_version()I",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_load_gtk_icon_linux_ge_v11() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = load_gtk_icon_linux_ge_v11(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
            "sun/awt/UNIXToolkit.load_gtk_icon(Ljava/lang/String;)Z",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_native_sync_linux_ge_v11() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = native_sync_linux_ge_v11(thread, Parameters::default());
        assert_eq!(
            "sun/awt/UNIXToolkit.nativeSync()V",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_unload_gtk_linux_ge_v11() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = unload_gtk_linux_ge_v11(thread, Parameters::default());
        assert_eq!(
            "sun/awt/UNIXToolkit.unload_gtk()Z",
            result.unwrap_err().to_string()
        );

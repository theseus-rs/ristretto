...
pub fn set_resizable<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn set_title<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn create_awt_window<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn get_native_window_size<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_screen_im_on<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(
...
pub fn get_sys_icon_height<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_sys_icon_width<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_sys_min_height<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_sys_min_width<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_sys_sm_icon_height<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn get_sys_sm_icon_width<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn modal_disable<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn modal_enable<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _blocker = parameters.pop_reference()?;
...
pub fn native_grab<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(
...
pub fn native_ungrab<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(
...
pub fn reposition_security_warning<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
pub fn request_window_focus<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn reshape_frame<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_always_on_top_native<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_focusable_window<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_full_screen_exclusive_mode_state<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_icon_images_data<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_min_size<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let _h = parameters.pop_int()?;
...
pub fn set_opacity<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn set_opaque_impl<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn update_insets<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
pub fn update_window_impl<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_set_resizable() {
        ...
        let result = set_resizable(thread, Parameters::new(vec![Value::from(false)]));
        assert_eq!(
    ...
    async fn test_set_title() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = set_title(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
    ...
    async fn test_create_awt_window() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = create_awt_window(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
            "sun/awt/windows/WWindowPeer.createAwtWindow(Lsun/awt/windows/WComponentPeer;)V",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_get_native_window_size() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_native_window_size(thread, Parameters::default());
        assert_eq!(
            "sun/awt/windows/WWindowPeer.getNativeWindowSize()Ljava/awt/Dimension;",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_get_screen_im_on() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_screen_im_on(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_sys_icon_height() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_sys_icon_height(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_sys_icon_width() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_sys_icon_width(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_sys_min_height() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_sys_min_height(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_sys_min_width() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_sys_min_width(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_sys_sm_icon_height() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_sys_sm_icon_height(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_sys_sm_icon_width() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_sys_sm_icon_width(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_modal_enable() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = modal_enable(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(
    ...
    async fn test_native_grab() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = native_grab(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_native_ungrab() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = native_ungrab(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_reposition_security_warning() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = reposition_security_warning(thread, Parameters::default());
        assert_eq!(
            "sun/awt/windows/WWindowPeer.repositionSecurityWarning()V",
            result.unwrap_err().to_string()
        );
    ...
    async fn test_request_window_focus() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = request_window_focus(thread, Parameters::new(vec![Value::from(false)]));
        assert_eq!(
    ...
    async fn test_set_always_on_top_native() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = set_always_on_top_native(thread, Parameters::new(vec![Value::from(false)]));
        assert_eq!(
    ...
    async fn test_set_focusable_window() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = set_focusable_window(thread, Parameters::new(vec![Value::from(false)]));
        assert_eq!(
    ...
    async fn test_set_min_size() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = set_min_size(thread, Parameters::new(vec![Value::Int(0), Value::Int(0)]));
        assert_eq!(
    ...
    async fn test_set_opacity() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = set_opacity(thread, Parameters::new(vec![Value::Int(0)]));
        assert_eq!(
    ...
    async fn test_set_opaque_impl() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = set_opaque_impl(thread, Parameters::new(vec![Value::from(false)]));
        assert_eq!(
    ...
    async fn test_update_insets() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = update_insets(thread, Parameters::new(vec![Value::Object(None)]));
        assert_eq!(

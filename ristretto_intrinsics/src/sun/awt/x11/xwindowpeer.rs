...
pub fn get_jvm_pid<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn get_local_hostname<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
...
mod tests {
    ...
    async fn test_get_jvm_pid() {
        ...
        let result = get_jvm_pid(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_get_local_hostname() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = get_local_hostname(thread, Parameters::default());
        assert_eq!(

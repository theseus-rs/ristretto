...
pub fn initialize_com<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn uninitialize_com<T: Thread + 'static>(
    _thread: Arc<T>,
    _parameters: Parameters,
) -> Result<Option<Value>> {
    Err(JavaError::UnsatisfiedLinkError(
...
mod tests {
    ...
    async fn test_initialize_com() {
        ...
        let result = initialize_com(thread, Parameters::default());
        assert_eq!(
    ...
    async fn test_uninitialize_com() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = uninitialize_com(thread, Parameters::default());
        assert_eq!(

...
pub async fn get_file_information_by_handle0<T: Thread + 'static>(
    ...
    let (vol_serial, n_links, file_index) =
        file_identity(vm.file_handles(), handle).await.unwrap_or((0, 1, 0));
...
pub async fn get_volume_information0<T: Thread + 'static>(
    ...
    let Reference::Object(ref mut obj) = *guard else {
        return Err(InternalError("GetVolumeInformation0: not an object".to_string()));
    };
...
fn volume_information(path: &str) -> Option<(String, String, u32, u32)> {
    ...
    let vol_len = vol_name.iter().position(|&c| c == 0).unwrap_or(vol_name.len());

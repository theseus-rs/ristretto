...
pub async fn init_1<T: Thread + 'static>(
    ...
    let [device, inode, ..] = values.as_mut() else {
...
pub async fn init_2<T: Thread + 'static>(
    ...
    let [serial_value, index_high, index_low, ..] = values.as_mut() else {
        return Err(InternalError("FileKey.init: finfo length < 3".to_string()));
    };
    #[expect(clippy::cast_possible_wrap)]
    {
        *serial_value = serial as i32;
        *index_high = (index >> 32) as i32;
        *index_low = (index & 0xFFFF_FFFF) as i32;
    }
...
mod wasm_tests {
    ...
    async fn test_init_ids() {
        let (_vm, thread) = crate::test::java21_thread().await.expect("thread");
        let result = init_ids(thread, Parameters::default()).expect("ok");
        assert_eq!(result, None);
    }
}

...
pub fn address_family<T: Thread + 'static>(
    _thread: Arc<T>,
...
pub fn is_connected<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let fd = parameters.pop_int()?;
...
pub fn peer_port_0<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let fd = parameters.pop_int()?;
...
pub fn so_type_0<T: Thread + 'static>(
    _thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let fd = parameters.pop_int()?;
...
pub fn unix_peer_address_0<T: Thread + 'static>(
    thread: Arc<T>,
    mut parameters: Parameters,
) -> Result<Option<Value>> {
    let fd = parameters.pop_int()?;
...
mod tests {
    ...
    async fn test_peer_port_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = peer_port_0(thread, Parameters::new(vec![Value::Int(-1)])).expect("peer port");
    ...
    async fn test_so_type_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = so_type_0(thread, Parameters::new(vec![Value::Int(-1)])).expect("socket type");
    ...
    async fn test_address_family() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = address_family(thread, Parameters::new(vec![Value::Int(-1)])).expect("family");
        assert_eq!(Some(Value::Int(-1)), result);
    }
    ...
    async fn test_is_connected() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result =
            is_connected(thread, Parameters::new(vec![Value::Int(-1)])).expect("connected");
    ...
    async fn test_unix_peer_address_0() {
        let (_vm, thread) = crate::test::thread().await.expect("thread");
        let result = unix_peer_address_0(thread, Parameters::new(vec![Value::Int(-1)]));
        assert_eq!(Some(Value::Object(None)), result.expect("peer address"));
    }
}

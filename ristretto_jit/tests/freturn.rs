...
fn freturn() -> Result<()> {
    ...
    let test_name_index = constant_pool.add_utf8("freturn")?;
    let test_descriptor_index = constant_pool.add_utf8("(F)F")?;
    ...
    let test_method_code = vec![Instruction::Fload_0, Instruction::Freturn];

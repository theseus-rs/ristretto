...
fn dreturn() -> Result<()> {
    ...
    let class_name_index = constant_pool.add_class("Double")?;
    let code_index = constant_pool.add_utf8("Code")?;
    let test_name_index = constant_pool.add_utf8("dreturn")?;
    let test_descriptor_index = constant_pool.add_utf8("(D)D")?;
    ...
    let test_method_code = vec![Instruction::Dload_0, Instruction::Dreturn];

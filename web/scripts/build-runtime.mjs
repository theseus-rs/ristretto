...
const { files } = await transpileBytes(
  readFileSync(join(root, 'target/wasm32-wasip2/release/ristretto_playground_engine.wasm')),
  {
    name: 'runner',
    instantiation: 'async',
    asyncMode: 'sync',
...
for (const { major, version } of jdks) {
  ...
  if (major !== 8) {
    ...
    // Compile against Java 11 so the same bridge source works on every JShell runtime.
    const bridgeDir = join(root, 'target', `playground-jshell-${major}`);
    rmSync(bridgeDir, { recursive: true, force: true });
    mkdirSync(bridgeDir, { recursive: true });
    run(join(dirname(jlink), 'javac'), [
      '--release',
      '11',
      '-d',
      bridgeDir,
      join(playground, 'runner/java/BrowserJShell.java'),
    ]);
    const bridgeFiles = {};
    for (const name of readdirSync(bridgeDir).sort()) {
      bridgeFiles[name] = [
        new Uint8Array(readFileSync(join(bridgeDir, name))),
        { mtime: new Date('2026-01-01T00:00:00Z') },
      ];
    }
    writeFileSync(join(imageDir, 'browser-jshell.jar'), zipSync(bridgeFiles));
  }
  const runtimeFiles = {};
  ...
  const names =
    major === 8
      ...
      : ['release', 'lib/modules', 'lib/tzdb.dat', 'conf', 'legal', 'browser-jshell.jar'];

...
mod tests {
    ...
    #[tokio::test]
    async fn test_iaload_invalid_value() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(2);
    ...
    #[tokio::test]
    async fn test_iaload_invalid_index_negative() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(2);
        let reference = Reference::from(vec![42i32]);
        let array = Value::new_object(thread.vm()?.garbage_collector(), reference);
    ...
    #[tokio::test]
    async fn test_iaload_invalid_index() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(2);
        let reference = Reference::from(vec![42i32]);
        let array = Value::new_object(thread.vm()?.garbage_collector(), reference);
    ...
    #[tokio::test]
    async fn test_iastore() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(3);
        let reference = Reference::from(vec![3i32]);
        let array = Value::new_object(thread.vm()?.garbage_collector(), reference);
        stack.push(array)?;
        stack.push_int(0)?;
        stack.push_int(42)?;
    ...
    #[tokio::test]
    async fn test_iastore_invalid_value() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(3);
        let reference = Reference::from(vec![42i8]);
        let object = Value::new_object(thread.vm()?.garbage_collector(), reference);
    ...
    #[tokio::test]
    async fn test_iastore_invalid_index_negative() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(3);
        let reference = Reference::from(vec![3i32]);
        let array = Value::new_object(thread.vm()?.garbage_collector(), reference);
    ...
    #[tokio::test]
    async fn test_iastore_invalid_index() -> Result<()> {
        let (_vm, thread) = crate::test::thread().await?;
        let stack = &mut OperandStack::with_max_size(3);
        let reference = Reference::from(vec![3i32]);
        let array = Value::new_object(thread.vm()?.garbage_collector(), reference);

...
pub(crate) async fn deflate_bytes(
    ...
    let Some(context) = map.get_mut(&handle) else {
        return Err(
            crate::JavaError::RuntimeException("Deflater has been closed".to_string()).into(),
        );
    };
...
pub(crate) async fn deflate_bytes_bytes(
    ...
    let input_bytes: Vec<u8> = {
        ...
        if in_len == 0 {
            ...
            return Err(crate::JavaError::ArrayIndexOutOfBoundsException {
...
#[async_method]
pub(crate) async fn end(_thread: Arc<Thread>, mut parameters: Parameters) -> Result<Option<Value>> {
    let handle = parameters.pop_long()?;
    let deflaters = get_or_init_deflaters();
    let mut guard = deflaters.write();
...
mod tests {
    ...
    async fn create_deflater(thread: &Arc<Thread>, level: i32, nowrap: bool) -> Result<i64> {

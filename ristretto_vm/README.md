...
through the Ristretto classloader and embeds the JDK Java compiler.
...
- Reusable file and memory-only Java compilation APIs.
...
Java sources can be compiled directly from memory:
...
async fn main() -> Result<(), CompilerError> {
    ...
    let classes = compiler
        .compile_source("Hello", "public class Hello {}")
        .await?;
    assert!(classes.get("Hello").is_some());
    Ok(())
}
```

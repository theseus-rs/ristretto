...
/// resolved package-to-module map.
fn create_named_boot_module<'a, T: Thread + 'static>(
    thread: &'a T,
    module_name: &'a str,
    package: &'a str,
) -> crate::BoxFuture<'a, Result<Value>> {
...
fn create_minimal_module_descriptor<'a, T: Thread + 'static>(
    thread: &'a T,
    module_name: &'a str,
    package: &'a str,
) -> crate::BoxFuture<'a, Result<Value>> {
...
fn system_module_descriptor<'a, T: Thread + 'static>(
    thread: &'a T,
    module_name: &'a str,
) -> crate::BoxFuture<'a, Option<ModuleDescriptor>> {
    Box::pin(async move {
        let vm = thread.vm().ok()?;
        ...
        let class_loader = class_loader.read().await.clone();
        class_loader
            .read_module_descriptor(&jimage_path, module_name)
            .await
            .ok()
    })
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_class_to_object_creates_named_boot_module() -> Result<()> {
        let package_map =
            AHashMap::from_iter([("named/package".to_string(), "named.module".to_string())]);
        let configuration = ResolvedConfiguration::new(
            std::collections::BTreeMap::new(),
            package_map,
            AHashMap::default(),
            AHashMap::default(),
        );
        let vm = test_utils::MockVm::with_resolved_configuration(JAVA_17, configuration);
        let thread = test_utils::MockThread::new(vm.clone());

        let class = test_utils::class("named/package/First", &[])?;
        assert!(class.to_object(&*thread).await?.is_object());

        let defined_module = vm
            .module_system()
            .get_module("named.module")
            .expect("named module");
        assert!(defined_module.module_object.is_some());
        assert!(defined_module.packages.contains("named.package"));

        let second_class = test_utils::class("named/package/Second", &[])?;
        assert!(second_class.to_object(&*thread).await?.is_object());
        assert_eq!(
            defined_module.module_object,
            vm.module_system()
                .get_module("named.module")
                .and_then(|module| module.module_object)
        );
